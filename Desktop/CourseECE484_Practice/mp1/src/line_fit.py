...
def line_fit(binary_warped):
	...
	for window in range(nwindows):
		...
		win_start = (leftx_current - margin,window_height*window)
		win_end = (rightx_current + margin,window_height*(window+1))
		
		# Draw the windows on the visualization image using cv2.rectangle()
		
		cv2.rectangle(out_img,win_start,win_end,(0,255,0),1)
		# cv2.imshow('window',out_img) # for debugging
		
		
		# Identify the nonzero pixels in x and y within the window
		nonzero = binary_warped[win_start[1]:win_end[1],win_start[0]:win_end[0]].nonzero()
		nonzeroy = np.array(nonzero[0])
		nonzerox = np.array(nonzero[1])
		
		# Append these indices to the lists
		left_lane_inds.append(nonzerox)
		right_lane_inds.append(nonzeroy)

		# If you found > minpix pixels, recenter next window on their mean position
		if len(nonzerox) > minpix:
			leftx_current = np.mean(nonzerox)
			rightx_current = np.mean(nonzeroy) 
